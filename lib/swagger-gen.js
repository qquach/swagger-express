...
function getPostParams(params, type){
  ...
  if(type=="urlencoded"){
    ...
    if(typeof(params)=="string"){
      ...
      if(arr.length==1){
        var schema = wsUtil.getSchema(params);
        id = arr[0];
        swaggerObj.definitions[id] = {"properties":schema};
      }
      else{//assume the definitions is the first aray value
        id = arr[1];
      }
    }else{
      id = randomName();
